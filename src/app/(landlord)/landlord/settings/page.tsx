...
function LandlordSettingsContent() {
  ...
  const handleProfileUpdate = (updated: ProfileData) => {
    setProfile(updated);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'PROFILE' && (
        <ProfileTab profile={profile} csrf={csrf} onProfileUpdate={handleProfileUpdate} />
      )}
      ...
      {activeTab === 'VERIFICATION' && (
        <VerificationTab profile={profile} csrf={csrf} onProfileUpdate={handleProfileUpdate} />
      )}
...
function ProfileTab({ profile, csrf, onProfileUpdate }: ProfileTabProps) {
  ...
  async function handleProfileSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (json.success) onProfileUpdate(json.data);
    } finally {
...
function VerificationTab({ profile, csrf, onProfileUpdate }: VerificationTabProps) {
  ...
  async function handleKycSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setFile(null);
      onProfileUpdate({ ...profile, kyc_status: 'PENDING' });
    } finally {
      setUploading(false);
    }
  }

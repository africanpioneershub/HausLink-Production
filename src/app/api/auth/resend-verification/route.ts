...
export async function POST(request: Request) {
  ...
  try {
    const { error } = await supabasePublicAuth.auth.resend({
      type: 'signup',
      email,
    ...
    if (error) {
      console.error('[resend-verification] Supabase resend returned an error', { email, error });
      return NextResponse.json(
    ...
    console.error('[resend-verification] Supabase resend threw an exception', { email, error });
    return NextResponse.json(

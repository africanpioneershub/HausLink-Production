...
export async function POST(request: Request) {
  ...
  await prisma.$transaction([
    prisma.payment.update({
      ...
      data: {
        status: 'COMPLETED',
        paid_at: new Date(),
        txn_ref: financialTransactionId ?? payment.txn_ref,
      },
    }),
  ...
  const transactionRef = financialTransactionId ?? payment.id;
  ...
  sendRentPaidEmail({
    ...
    amount: grossAmount,
    transactionRef,
  }).catch((error) => console.error('[momo webhook] Email failed', error));
  ...
  if (payment.tenant.phone) {
    ...
    }).catch((error) => console.error('[momo webhook] Tenant WhatsApp failed', error));
  }
  ...
  if (payment.landlord.phone) {
    ...
    }).catch((error) => console.error('[momo webhook] Landlord WhatsApp failed', error));
  }
